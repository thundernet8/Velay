...
function collectData(vm, data) {
    var dataValues = typeof data === 'function' ? data.call(vm, vm) : data || {};
    var filteredValues = Object.keys(dataValues).reduce(function (prev, curr) {
        var value = dataValues[curr];
...
function reactiveComponent(name, Component) {
    ...
    Component.prototype.$mount = function () {
        ...
        var reaction = new mobx.Reaction(name + ".render()", reactiveRender);
...
// tslint:disable-next-line:no-empty
var noop$2 = function () { };
function getComponentName(options, component) {
...
function injectService(Component, options) {
    var beforeCreate = options.beforeCreate || noop$2;
...
function Component(options) {
    var originalOptions = typeof options === 'function' ? options.options : options;
    var combineOptions = __assign({}, originalOptions, { data: function (vm) { return collectData(vm, originalOptions.data); }, _Ctor: {} });
    if (typeof options === 'function') {
        var name_1 = getComponentName(options);
        var target = getInjectedConstructor(options);
        if (!target) {
            throw new Error(name_1 + " dependency injection failed");
        }
        target.__decorators__ = options.__decorators__;
        return reactiveComponent(name_1, componentFactory(target, injectService(target, __assign({}, combineOptions, { name: name_1 }))));
    }
    return function (Component) {
        var name = getComponentName(options, Component);
        ...
        return reactiveComponent(name, componentFactory(target, injectService(target, __assign({}, combineOptions, { name: name }))));
    };
}
