...
function handleStoreServiceBinding(service, vm) {
    ...
    if (!vuexStore) {
        vuexStore = vm.$velayStore = new Vuex.Store({
            state: {}
        });
    }
    ...
    install: function (_Vue) {
        _Vue.mixin({
            beforeCreate: function () {
                ...
                if (!velayStore) {
                    if (options.store) {
                        velayStore = typeof options.store === 'function' ? options.store() : options.store;
                    }
                ...
                // service injection
                var vid = this.$options._vid;
                var instance = componentInstanceMap[vid];
                if (!instance) {
                    return;
                }
