...
function collectData(vm, data) {
    ...
    const filteredValues = Object.keys(dataValues).reduce((prev, curr) => {
...
function reactiveComponent(name, Component) {
    ...
    Component.prototype.$mount = function (...args) {
        ...
        const reaction = new mobx.Reaction(`${name}.render()`, reactiveRender);
...
function Component(options) {
    const originalOptions = typeof options === 'function' ? options.options : options;
    const combineOptions = {
        ...originalOptions,
        data: (vm) => collectData(vm, originalOptions.data),
        _Ctor: {}
    };
    if (typeof options === 'function') {
        ...
        return reactiveComponent(name, componentFactory(target, { ...combineOptions, name }));
    }
